...
from . import vit_seg_configs_cls as configs
import timm
...
class CoordAttention(nn.Module):
    """Coordinate Attention from 'CoordAttention: ...'"""
...
#      EffTransUNet_cls
# =====================================
class EffTransUNet_cls(nn.Module):
    def __init__(self, config_name='EfficientNet-B3', img_size=224, num_classes=4, vis=False):
        super(EffTransUNet_cls, self).__init__()

        config = CONFIGS[config_name]
        self.transformer = Transformer(config, img_size, vis)
        self.decoder     = DecoderCup(config, img_size)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        last_ch = config.decoder_channels[-1]
